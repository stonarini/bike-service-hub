	...
	return {
			...
			return connection.DB.collection(mainModel)
				.aggregate([
						...
						$set: {
							[`${relationModel}s`]: {
								$map: {
									input: `$${relationModel}s`,
									in: {
										$mergeObjects: ["$$this", { _id: `$$this.${relationModel}_id` }],
									},
								},
							},
						},
					},
					{
						$project: {
							info: 0,
							[`${relationModel}s.${mainModel}_id`]: 0,
							[`${relationModel}s.${relationModel}_id`]: 0,
						},
					},
				])
				.toArray();
		...
		findById: function (id) {
			return new Promise(async res => {
				res((await this.find({ _id: new ObjectId(id) }))[0]);
			});
		},
		create: obj => {
